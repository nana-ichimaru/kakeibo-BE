...
def get_cash_flows_by_month(
    ...
    stmt = (
        ...
        .where(
            CashFlow.recorded_at >= month_start_date, CashFlow.recorded_at < next_month_start_date
        )
    )
...
def get_cash_flow_by_id(session: Session, cash_flow_id: int) -> CashFlow | None:
    result: Result = session.execute(select(CashFlow).where(CashFlow.id == cash_flow_id))
